    	...
    	<div className={"radio-row "+injectData.required}>
    		...
    		<div className={injectData.readOnly?"radio-row-radio read-only":"radio-row-radio"}>
    		{
    			injectData.enumOption.map((item,key) =>{
    				return (<span key={key} name = { injectData.name } onClick    = { this.changeHandler } className={ injectData.value == item.key ? (key=='0'?'man item cur':'women item cur') : (key=='0'?'man item':'women item')} value = {item.key}>{item.value}</span>)
    			})
    		}
    		</div>
    	</div>
