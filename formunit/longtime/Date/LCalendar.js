let LCalendar = function () {
      ...
      MobileCalendar.prototype = {
        init: function (params) {
          ...
          this.bindEvent(this.type, params.name);
        },
        bindEvent: function (type, name) {
          var _self = this;
          //呼出日期插件
          function popupDate(e) {
            ...
            lcalendar_finish.addEventListener('click', function (e) { finishMobileDate(e, _self) });
          ...
          function dateCtrlInit() {
            ...
            if (_self.trigger.getAttribute("data-lcalendar") != null) {
              var arr = _self.trigger.getAttribute("data-lcalendar").split(',')[0];
              var setArr = new Date(arr)
              date = setArr > date ? setArr : date
            }
            ...
            if (/^\d{4}-\d{1,2}-\d{1,2}$/.test(_self.trigger.value)) {
              ...
              dateArr.dd = rs[2].replace(/-/g, "") - 1;
            	...
            	//如果当前日期不在日期范围内,那么默认取范围内最小日期
            	if(dateArr.yy >= _self.minY && dateArr.yy <= _self.maxY){
          ...
          function popupYM(e) {
            _self.gearDate = document.createElement("div");
            _self.gearDate.className = name ==='yearDate' ? 'gearDate year':  "gearDate";
            _self.gearDate.innerHTML = '<div class="date_ctrl slideInUp">' +
              '<div class="date_btn_box">' +
              '<div class="date_btn lcalendar_cancel">取消</div>' +
              '<div class="date_btn lcalendar_finish">确定</div>' +
          ...
          function setDateGearTooth() {
            ...
            var date_yy = _self && _self.gearDate && _self.gearDate.querySelector(".date_yy") || '';
            var itemStr = "";
          ...
          //日期确认
          function finishMobileDate(e, obj) {
            var passY = _self.maxY - _self.minY + 1;
            ...
            _self.trigger.choiceValue = (date_yy % passY + _self.minY) + "-" + date_mm + "-" + date_dd;
            _self.onSuccess && _self.onSuccess(_self.trigger)
          ...
          function finishMobileYM(e) {
            ...
            var date_mm = parseInt(Math.round(_self.gearDate.querySelector(".date_mm").getAttribute("val"))) + 1;
            date_mm = date_mm > 9 ? date_mm : '0' + date_mm;
            _self.trigger.choiceValue = (date_yy % passY + _self.minY) + "-" + date_mm;
            _self.onSuccess && _self.onSuccess(_self.trigger)
            closeMobileCalendar(e);
          }
          //日期时间确认
          function finishMobileDateTime(e) {
            ...
            _self.trigger.choiceValue = (date_yy % passY + _self.minY) + "-" + date_mm + "-" + date_dd + " " + (time_hh.length < 2 ? "0" : "") + time_hh + (time_mm.length < 2 ? ":0" : ":") + time_mm;
            closeMobileCalendar(e);
          }
          //时间确认
          function finishMobileTime(e) {
            ...
            var time_mm = parseInt(Math.round(_self.gearDate.querySelector(".time_mm").getAttribute("val")));
            time_mm = time_mm > 9 ? time_mm : '0' + time_mm;
            _self.trigger.choiceValue = (time_hh.length < 2 ? "0" : "") + time_hh + (time_mm.length < 2 ? ":0" : ":") + time_mm;
            closeMobileCalendar(e);
          }
